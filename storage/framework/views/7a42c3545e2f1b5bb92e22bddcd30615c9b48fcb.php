                               ...
                               <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <ul class="list-unstyled mb-0" id="todayEventsList">
                                        <?php if(isset($todayEvents) && count($todayEvents) > 0): ?>
                                            <?php $__currentLoopData = $todayEvents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <li class="list-item mt-3 fw-semibold d-flex justify-content-between align-items-center today-event-item" 
                                                    ...
                                                    data-job-id="<?php echo e($event['job_id'] ?? ''); ?>"
                                                    data-client-id="<?php echo e($event['client_id'] ?? ''); ?>">
                                                    <span>
                                                        <i class="fas fa-circle font-10 me-2" style="color:<?php echo e($event['color'] ?? '#8FDCB2'); ?>"></i><?php echo e($event['title']); ?>
      ...
      document.querySelectorAll('.today-event-item').forEach(function(item) {
        item.addEventListener('click', function() {
          ...
          if (eventType === 'job' && jobId) {
            window.location.href = '<?php echo e(url('/')); ?>/jobs/' + jobId;
          } else if (clientId) {
            window.location.href = '<?php echo e(url('/')); ?>/clients/' + clientId;
          }
        });
      });
        ...
        if (calendarEl) {
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: function(info, successCallback, failureCallback) {
                    ...
                    fetch('<?php echo e(url('/')); ?>/api/calendar-events')
                ...
                eventClick: function(info) {
                    ...
                    if (info.event.extendedProps.type === 'job' && info.event.extendedProps.job_id) {
                        window.location.href = '<?php echo e(url('/')); ?>/jobs/' + info.event.extendedProps.job_id;
                    } else if (info.event.extendedProps.client_id) {
                        window.location.href = '<?php echo e(url('/')); ?>/clients/' + info.event.extendedProps.client_id;
                    }
                }
            });
            calendar.render();
...
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u246429533/domains/gemdentalrepair.com/public_html/crm/resources/views/calendar.blade.php ENDPATH**/ ?>