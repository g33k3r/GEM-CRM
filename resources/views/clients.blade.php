                                    ...
                                    <div class="table-responsive">
                                        <table class="table" id="datatable_1">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <span class="tble-clr">{{ $client->created_at ? $client->created_at->format('m/d/y') : 'N/A' }}</span>
                                                        <p class="mb-0 tble-time">{{ $client->created_at ? $client->created_at->format('h:i a') : '' }}</p>
                                                    </td>
                                                    <td onclick="window.location.href='{{ url('clients/' . $client->id) }}'" style="cursor: pointer;">
                                                        <div class="d-flex align-items-center justify-content-between">
                                                            <div>
                                                                <span class="tble-clr">{{ $client->office_name ?? 'N/A' }}</span>
                                                                <p class="mb-0 tble-time">Contact Number: {{ $client->office_number ?? 'N/A' }}</p>
                                                            </div>
                                                    ...
                                                    <td onclick="window.location.href='{{ url('clients/' . $client->id) }}'" style="cursor: pointer;">
                                                        <div class="d-flex align-items-center justify-content-between">
                                                            <div>
                                                                <span class="tble-clr">{{ $client->doctor_name ?? 'N/A' }}</span>
                                                                <p class="mb-0 tble-time">Contact Number: {{ $client->office_number ?? 'N/A' }}</p>
                                                            </div>
                                                            
                                                        </div>
                                                    </td>
                                                    <td><span class="tble-clr">{{ $client->city ?? 'N/A' }}</span></td>
                                                    <td><span class="tble-clr">{{ $client->zip_code ?? 'N/A' }}</span></td>
                                                    <td><span class="tble-clr">{{ $client->address ?? 'N/A' }}{{ $client->suite ? ', Suite ' . $client->suite : '' }}{{ $client->state ? ', ' . $client->state : '' }}</span></td>
                                                    <td>
                                                        <span class="tble-clr">{{ $client->updated_at ? $client->updated_at->format('m/d/y') : 'N/A' }}</span>
                                                        <p class="mb-0 tble-time">{{ $client->updated_at ? $client->updated_at->format('h:i a') : '' }}</p>
                                                    </td>
                                                    <td>
                                                        <span class="{{ $client->status == 'active' ? 'active-status' : ($client->status == 'inactive' ? 'inactive-status' : 'deactive-status') }}">
                                                    ...
                                                    <td>
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="dropdown-item" href="{{ url('clients/' . $client->id) }}"><i class="ti ti-eye" style="margin-right:5px"></i> View</a>
                                                            <a class="dropdown-item delete-client-btn" href="#" data-client-id="{{ $client->id }}" data-office-name="{{ $client->office_name ?? 'N/A' }}"><i class="ti ti-trash" style="margin-right:5px"></i> Delete</a>
                                                        </div>
                                                    </td>
                                                </tr>
                                                @empty
            ...
            if (deleteClientModalElement) {
                ...
                $('#confirmDeleteClient').on('click', function() {
                    if (clientIdToDelete) {
                        ...
                        // Make AJAX request
                        $.ajax({
                            url: `{{ url('/') }}/clients/${clientIdToDelete}`,
                            method: 'DELETE',
                            headers: {
